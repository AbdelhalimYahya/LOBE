...
function HairCategoryPageContent() {
  ...
  // URL Params State (Source of Truth for Fetching)
  ...
  useEffect(() => {
    const read = () => {
      try {
        ...
        setSearchInput(search); // Sync input with URL initially
  ...
  useEffect(() => {
    async function fetchBrands() {
      try {
        const res = await authenticatedFetch(`${API_BASE_URL}/v1/haircare/select_brands/`);
        if (res.ok) {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        let url = `${API_BASE_URL}/v1/haircare/haircare_products/?page=${page}&size=${size}`;
  ...
  const buildUrl = (newPage: string | number, newBrand?: string, newSearch?: string) => {
    ...
    // Check search
    const searchVal = newSearch !== undefined ? newSearch : searchInput; // Use current input for new builds
  ...
  const handleSearchKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      e.preventDefault();
      // This triggers URL update -> popstate/useEffect read -> updating activeSearchQuery -> fetch
      setPageParam("1");
      setActiveSearchQuery(searchInput); // Optimistic update
            ...
            <div className="rounded-3xl bg-white/85 backdrop-blur-md border border-pink-50 shadow-[0_16px_35px_rgba(244,114,182,0.18)] px-3.5 py-4 md:px-4 md:py-5 flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2.5 text-right">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setPageParam("1");
                      // When filtering by brand, keep the search query
                ...
                <div className="flex flex-col gap-1 opacity-50 cursor-not-allowed">
                  <label className="text-[11px] text-slate-500">
                    أقل درجة أمان (غير متاح حالياً)
                  </label>
                  <select
                    value={minSafety}
                    disabled
                    className="w-full rounded-2xl border border-pink-100 bg-white px-3 py-1.5 text-[11px] text-right outline-none bg-gray-100 cursor-not-allowed"
                  >
          ...
          <section className="flex-1 flex flex-col gap-6 min-h-[500px]">
            {loading ? (
              ...
              <>
                {products.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                    {products.map((product) => {
                      if (hiddenProducts.has(String(product.haircare_id))) return null;
                        ...
                        >
                          <div className="relative w-full aspect-[3/4] bg-gradient-to-b from-[#fff9fc] via-[#fff5f9] to-[#ffe9f3] flex items-center justify-center overflow-hidden">
                            {product.image_url ? (
                              <img
                                ...
                                onError={() => {
                                  setHiddenProducts(prev => {
                                    ...
                                    next.add(String(product.haircare_id));
                                    return next;
                                  });
                                }}
                              ...
                              <div className="flex flex-col items-center justify-center gap-1 text-[11px] text-muted-foreground w-full h-full">
                ...
                {/* Pagination */}
                {serverTotalPages > 1 && (
                  <nav className="mt-2 flex justify-center items-center gap-2 text-sm md:text-base transition-opacity duration-300">
                    {generatePageNumbers(currentPage, serverTotalPages).map((item, idx) => {
                      if (item === "dots") {
                        return (
                          <span key={`dots-${idx}`} className="min-w-[36px] h-9 flex items-center justify-center text-xs md:text-sm text-muted-foreground">...</span>
                        );
                      }
